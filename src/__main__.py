...
for input in raw_inputs:
    ...
    paramater_type_dict = {
        parameter: _function["parameters"][parameter]["type"]
        for parameter in _function["parameters"]
    }
    prompt = f"We will use the function {function_name} and its parameters."
    prompt += f"Identify in the following prompt each parameter value and "
    prompt += f"associate it to a presented parameter. The formatted "
    prompt += r'parameters must be : "{name: value}" '
    prompt += "and it must respect the JSON schema rules.\n"
    prompt += f"Prompt: {input}\n"
    prompt += f"{function_name} parameters: {str(paramater_type_dict).replace("'", "")}.\n"
    prompt += 'Formatted parameters: {"'
    ...
    for i in range(50):
        ...
        next_token_decoded = model.decode(best_id)
        sys.stdout.write(next_token_decoded)
